...
elseif($project.ExtenderNames -contains "WebApplication") {
	...
	if($depAsm) {
		...
		if($amd64) {
			...
			$crt64 = Ensure-Folder $amd64 "Microsoft.VC110.CRT";
			if($crt64) {
				$crt64dir = (Join-Path $amd64dir "Microsoft.VC110.CRT")
				Add-ProjectItem $crt64 (Join-Path $crt64dir "msvcp110.dll");
				Add-ProjectItem $crt64 (Join-Path $crt64dir "msvcr110.dll");
				Add-ProjectItem $crt64 (Join-Path $crt64dir "vccorlib110.dll");
			}
		}
		$x86 = Ensure-Folder $depAsm "x86";
		if($x86) {
			$x86dir = (Join-Path $nativeBinDirectory "x86")
			$crt32 = Ensure-Folder $x86 "Microsoft.VC110.CRT";
			if($crt32) {
				$crt32dir = (Join-Path $x86dir "Microsoft.VC110.CRT")
				Add-ProjectItem $crt32 (Join-Path $crt32dir "msvcp110.dll");
				Add-ProjectItem $crt32 (Join-Path $crt32dir "msvcr110.dll");
				Add-ProjectItem $crt32 (Join-Path $crt32dir "vccorlib110.dll");
			}
		}
	}
}
